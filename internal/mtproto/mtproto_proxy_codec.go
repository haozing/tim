...
// 协议的完整版本、中间版本和精简版本都支持快速确认。
// 在这种情况下，客户端设置查询数据包中的最高顺序长度位，服务器用一个特殊的4字节作为单独的数据包进行响应。
// 它们是数据包加密部分的32个高顺序位SHA256，
// 由来自授权密钥的32字节（与为验证消息密钥而计算的哈希相同），
// 其中最重要的位设置为明确这不是常规服务器响应包的长度；
// 如果使用简略版本，则对这4个字节应用bswap。
...
func (c *MTProtoProxyCodec) peekCodec() error {
	...
	glog.Info("mtproto abridged version.b_0_1[0]",b_0_1[0])
